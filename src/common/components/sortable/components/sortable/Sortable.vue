...
import { HostsManager, EventsManager } from '../../manager';
import { closest } from '../../utils';
import { EventScheduler, MoveEvent, StartEvent, EndEvent, ChangeEvent, DropEvent } from '../../events';
...
export default {
	...
	setup(props, { slots, emit }) {
		...
		/**
		 * Returns information like HTMLElement, index and other useful info from an HTMLElement
		 *
		...
		return () => {
			...
			return h(
				props.tag,
				{
					...
					class: {
						[`vuebdnd__placeholder-empty-container`]:
							childItems.length === 0 || (dragging.value && childItems.length === 1),
					},
				},
				[childElements],
			);
