<template>
	<div class="znpb-input-number-unit" :title="$attrs.title">
		<BaseInput
			ref="numberUnitInput"
		...
		>
			<template #suffix>
				<Icon class="znpb-input-number__dots" icon="three-dots" @click="expanded = !expanded" />
			</template>
		</BaseInput>

		<div class="znpb-input-number__units" :class="{ 'znpb-input-number__units-multiple': multipleUnits }">
			<Tooltip
				:trigger="null"
				:show="expanded"
				...
				<template #content>
					<div class="znpb-number-unit-list hg-popper-list">
						...
						<div
							...
							class="znpb-number-unit-list__option hg-popper-list__item"
							:class="{
								[`znpb-number-unit-list__option--selected`]:
									(valueUnit.unit === availableUnit || unit === availableUnit) &&
									(isValidUnit || units.includes(stringValueModel) || unit === availableUnit),
							}"
		...
		stringValueModel: {
			...
			set(newValue) {
				...
				if (
					...
					this.unit &&
					this.unit !== 'custom' &&
					this.units.includes(this.unit) &&
					newValue &&
					newValue.match(/^[0-9]*$/) !== null &&
					!stringUnits.includes(this.unit)
				) {
			...
			get() {
				const match =
					typeof this.modelValue === 'string' && this.modelValue
						? this.modelValue.match(/^([+-]?[0-9]+([.][0-9]*)?|[.][0-9]+)(\D+)$/)
						: null;
